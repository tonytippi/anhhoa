...
- Xe đưa đón và đón muộn tự tính khi chưa có mô-đun nguồn được phát hành. Xe đưa đón không thuộc release đầu; phí đón muộn vẫn có thể cấu hình `MANUAL`/`FIXED` cho tới khi có handover. Tiền ăn và thứ bảy là ngoại lệ trong phạm vi release đầu theo phần 6.4 và 6.5.
...
| `quantitySource` | `FIXED`, `MANUAL`, `ATTENDANCE_DAYS`, `MEAL_MONTHLY_LEAVE_DEDUCTION`, `SATURDAY_ATTENDANCE`, `LATE_PICKUP_BLOCKS`, `TRANSPORT_TRIPS` |
| `fixedQuantity` | Bắt buộc khi `FIXED`; số nguyên dương, mặc định 1 |
...
Quy tắc rõ ràng này tốt hơn nút gán mơ hồ: Admin có thể xem vì sao một học sinh nhận một khoản, và API có thể xác định precedence mà không dựa vào UI.
...
| `periodKey` | Unique trong loại kỳ: `2026-09`, `2026-2027`, hoặc UUID/tên sự kiện chuẩn hóa |
...
Một học sinh có tối đa một hóa đơn cho một `CollectionRun`: unique `(studentId, collectionRunId)`. `MONTHLY` dùng `periodKey` `YYYY-MM`; báo cáo tháng tiếp tục aggregate các đợt `MONTHLY` tương ứng. Khoản theo năm/sự kiện không còn bị ép vào unique `(studentId, billingMonth)`.

`CollectionRun` cần có `schoolYearLabel` snapshot/tùy chọn hiển thị, vì wizard Kidsonline gắn đợt thu với năm học. Đây là thuộc tính tổ chức/kỳ thu, không thay thế một mô-đun quản lý năm học đầy đủ. Run không giữ tài khoản nhận tiền bắt buộc: tài khoản là quyết định của từng invoice tại lúc issue.

### 5.5 Hóa đơn và dòng hóa đơn

Invoice dùng `collectionRunId` bắt buộc thay `billingMonth` đơn lẻ. Với compatibility đọc trên UI, invoice trả `periodType`, `periodKey`, `collectionRunName` snapshot. Lifecycle settlement mới được định nghĩa tại phần 7.3.
...
| `pricingSource`, `quantitySource` | Giải thích nguồn giá/số lượng |
| `sourceReference` nullable | Ví dụ danh sách ID/tổng hợp ngày của late pickup; không lộ dữ liệu không cần thiết cho Parent |
...
quantity =
  fixedQuantity                             khi quantitySource = FIXED
  giá trị Admin nhập trong phạm vi cho phép khi quantitySource = MANUAL
  số ngày nghỉ có phép hợp lệ do API tổng hợp khi quantitySource = MEAL_MONTHLY_LEAVE_DEDUCTION
  số attendance thứ bảy chargeable do API tổng hợp khi quantitySource = SATURDAY_ATTENDANCE
  số sự kiện hợp lệ do API tổng hợp          khi nguồn vận hành đã được phát hành
...
- `MEAL_MONTHLY_LEAVE_DEDUCTION` tạo dòng deduction âm từ số ngày nghỉ có phép hợp lệ; nó không thay thế quantity dương của khoản tiền ăn tháng gốc.

### 6.4 Tiền ăn, đơn nghỉ và hoàn trả theo khoản

`MEAL_MONTHLY_LEAVE_DEDUCTION` là policy release đầu cho tiền ăn tính theo tháng:

1. API bắt đầu từ số tiền/số lượng tiền ăn tháng đã được rule snapshot, không tính lại theo attendance thực tế từng ngày.
2. Parent tạo đơn nghỉ cho học sinh có link active, chọn effective date không trước ngày gửi. Đơn được gửi trước `15:00` giờ `Asia/Ho_Chi_Minh` của ngày lịch ngay trước ngày nghỉ đầu tiên tự duyệt (`AUTO_APPROVED`); một đơn nhiều ngày hợp lệ áp dụng cho toàn bộ ngày đã duyệt của đơn.
3. Đơn gửi từ `15:00` trở đi cho ngày nghỉ đầu tiên chuyển `PENDING_REVIEW` và chỉ tạo deduction sau khi `SCHOOL_ADMIN` hoặc `FINANCE_MANAGER` duyệt. Đơn bị từ chối và ngày nghỉ không phép không tạo deduction tiền ăn tự động.
4. API chỉ trừ số ngày thuộc tháng thu trong leave request được tự duyệt hoặc được duyệt thủ công, trừ ngày có attendance `PRESENT` được giáo viên xác nhận theo AttendancePolicy của School ở phần 6.5. Approval/rejection, thời điểm gửi, effective date, actor duyệt khi có, các ngày được áp dụng và số tiền trừ phải snapshot vào invoice `DRAFT`. Policy/đơn đổi sau `ISSUED` không sửa hóa đơn lịch sử; dùng adjustment/refund mới có liên kết nếu cần.
...
- Học sinh không đăng ký dịch vụ không nhận khoản trọn gói. Mỗi ngày thứ bảy hợp lệ theo SchoolCalendar có attendance `PRESENT` đã xác nhận tạo một dòng/phần số lượng `SATURDAY_ATTENDANCE` cho khoản học lẻ thứ bảy.
- Học sinh có đăng ký gói cả tháng chỉ được trừ khi có leave request đủ điều kiện và không có `PRESENT` đã xác nhận trong ngày trùng. Không charge học lẻ cho một ngày đã được bao phủ bởi đăng ký active. Attendance bị sửa sau `ISSUED` không đổi hóa đơn lịch sử; xử lý bằng adjustment/refund có nguồn và audit.
- School tự tạo các dịch vụ/khoản khác theo cùng cấu trúc; release đầu không có `TRANSPORT_TRIPS` hoặc xe đưa đón.
...
**Ranh giới module:** khoản giảm trừ/carryover là ledger entry trên invoice, còn `Phí đón muộn` là nguồn số lượng vận hành. Chúng có thể cùng xuất hiện là dòng hóa đơn, nhưng không dùng một module "giảm trừ" để ghi nhận sự kiện đón muộn hoặc dùng một danh mục khoản thu để che dấu khoản nợ kỳ trước.

### 6.7 Phí trông muộn

`LATE_PICKUP_BLOCKS` là source release đầu, do giáo viên xác nhận trả trẻ trong app. School cấu hình cutoff, grace và block; ví dụ cutoff `17:30`, không grace, block `30` phút. `LatePickupPolicy` có version/effective date nếu School thay đổi sau này. API tính từ `pickedUpAt` thực tế:

```text
chargeableMinutes = max(0, pickedUpAt - pickupCutoff - graceMinutes)
blocks = ceil(chargeableMinutes / blockMinutes)
latePickupFee = blocks x unitPrice
```

Ví dụ với cutoff `17:30` không grace, trả trẻ lúc `17:31` tạo một block 30 phút. Quy tắc cần được snapshot trên dòng: giờ đón chuẩn, grace period, block minutes, unit price, timestamp bàn giao và danh sách sự kiện nguồn. Bản ghi được miễn/đánh dấu lỗi không được đưa vào số lượng. Finance Manager/SCHOOL_ADMIN chỉ override trong `DRAFT`, có ghi chú và audit.

Không cho Finance tạo `LATE_PICKUP_BLOCKS` không có handover source; trường hợp ngoại lệ dùng adjustment thủ công trong `DRAFT` có ghi chú/audit. `Phí trông muộn` vẫn có thể cấu hình `MANUAL` hoặc `FIXED` cho trường không bật workflow trả trẻ.
...
- Filter theo học sinh/lớp và drill-down xem từng nguồn số lượng trước khi generate.
...
- Response phân biệt `created`, `skippedExistingInvoice`, `skippedInactiveStudent`, `skippedNoActiveClass`, `skippedNoApplicableRule`, `skippedInvalidDynamicSource`.
...
| `POST` | `/attendance/class-sessions/:id/records` | Class Teacher ghi attendance cho lớp phân công; `PRESENT` tuân theo `AttendancePolicy.photoEvidenceMode`, là source cho học lẻ thứ bảy |
| `POST` | `/handovers/:id/complete` | Class Teacher xác nhận `pickedUpAt` thực tế; source cho late-pickup blocks |
...
- Rule hiển thị rõ `Khoản thu`, phạm vi, nguồn giá, nguồn số lượng, giá/đơn vị dự kiến, thứ tự; không dùng biểu tượng không nhãn.
...
- Dòng dynamic cho biết ví dụ `3 block đón muộn từ 05/09, 12/09, 20/09`; Parent chỉ nhận tên, đơn vị, giá, số lượng và thành tiền cần thiết.
...
- Trang khoản tiền ăn tháng giải thích rõ số tiền gốc, ngày nghỉ có phép được trừ, auto-approval trước `15:00 ngày hôm trước`, review sau deadline và exception `PRESENT` đã xác nhận; không che deduction trong một tổng mơ hồ.
- Đăng ký dịch vụ có danh sách học sinh, hiệu lực, trạng thái và lịch sử. Invoice thứ bảy hiển thị rõ `Gói học thứ bảy` hoặc số buổi `Học lẻ thứ bảy` từ attendance `PRESENT` đã xác nhận, không cho một ngày xuất hiện cả hai khoản.
- Giáo viên có action `Trả trẻ` ghi thời điểm thực tế. Preview late-pickup hiển thị cutoff/block theo School policy, timestamp nguồn và số block; ví dụ cutoff `17:30` không grace thì `17:31` là một block 30 phút. Không để browser tự tính hoặc Parent tự xác nhận giờ trả trẻ.
...
| `MEAL` | Tiền ăn | Chung | School cấu hình | School cấu hình | tháng | `RECEIVABLE_DEFAULT` | `FIXED: 1` + deduction `MEAL_MONTHLY_LEAVE_DEDUCTION` |
...
| `SATURDAY_SINGLE` | Học lẻ thứ bảy | Dịch vụ | School cấu hình | School cấu hình | buổi | `RECEIVABLE_DEFAULT` | `SATURDAY_ATTENDANCE` khi không có service enrollment bao phủ |
| `LATE_PICKUP_30M` | Phí trông muộn | Chung | 20.000 | 20.000 | 30 phút | `RECEIVABLE_DEFAULT` | `MANUAL` cho đến khi có handover |
...
- Unit test meal deduction: leave request trước 15:00 tự duyệt, từ 15:00 chuyển review và chỉ trừ sau School Admin/Finance Manager duyệt; boundary đúng 15:00, effective date không trước ngày gửi, nghỉ dài hạn/hủy dịch vụ đủ điều kiện hoàn trả, `PRESENT` đã xác nhận override leave deduction và Finance Manager manual quantity/price override luôn cần ghi chú/audit, dù đã có source data.
- Unit test thứ bảy: service enrollment active tạo gói cố định, không đăng ký chỉ tính `PRESENT` đã xác nhận vào ngày calendar hợp lệ, không charge trùng một ngày đã được gói bao phủ và chỉ leave request đủ điều kiện không có `PRESENT` mới trừ gói.
- Unit test công thức đón muộn: với cutoff `17:30`/không grace, đúng `17:30` là 0, `17:31` là một block 30 phút; test thêm policy cutoff/grace/block khác, đúng biên block, làm tròn lên, miễn phí và event invalid.
...
5. Đã chốt: tiền ăn thu theo tháng, chỉ trừ ngày thuộc đơn nghỉ có phép được duyệt và Parent gửi trước 15:00 ngày trước ngày nghỉ đầu tiên; không tính tiền ăn theo attendance từng ngày.
6. Phí trông muộn có giờ cutoff/grace/block chung hay theo lớp; ai được miễn/override và ai duyệt?
...
12. Đã chốt: School Admin hoặc Finance Manager tạo/hủy `StudentServiceEnrollment` cho gói học thứ bảy theo thông báo/yêu cầu Parent, có effective dates/audit; Parent không tự thay đổi đăng ký hoặc hủy dịch vụ.
