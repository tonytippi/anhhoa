...
export function AdminShell() { const [session, setSession] = useState<Session | undefined>(); const [ready, setReady] = useState(false); const clear = () => setSession(undefined); useEffect(() => { const controller = new AbortController(); let current = true; void bootstrapSession('app', clear, controller.signal).then((result) => { if (current) { setSession(result); setReady(true); } }); return () => { current = false; controller.abort(); }; }, []); if (!ready) return <main><h1>PassionEdu - Quản trị trường</h1><p>Đang xác thực phiên...</p></main>; if (!session) return <main><h1>PassionEdu - Quản trị trường</h1><p>Vui lòng đăng nhập để tiếp tục.</p><a href={googleLoginUrl('app')}>Đăng nhập với Google</a></main>; return <main><a href="#school-content">Bỏ qua điều hướng</a><div id="school-content"><SchoolContext clear={clear} /></div><button onClick={() => void logout('app', clear)}>Đăng xuất</button></main>; }
