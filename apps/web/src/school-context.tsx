...
type View = "overview" | "students" | "parents" | "staff" | "classes" | "years" | "positions" | "settings" | "leave-review" | "finance" | "receipt-queue" | "finance-reports";
...
const pages: View[] = ["overview", "students", "parents", "staff", "classes", "years", "positions", "settings", "leave-review", "finance", "receipt-queue", "finance-reports"];
...
export function SchoolContext({ clear, userIdentityId }: { clear: () => void; userIdentityId: string }) {
  ...
  const refreshChooser = async (restoreSelection: boolean) => {
    ...
    if (restoreSelection && school) navigate(pathFor({ schoolSlug: school.schoolSlug, page: "overview" }), { replace: true });
    else {
