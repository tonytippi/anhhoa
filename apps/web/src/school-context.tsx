...
const pages: View[] = ["overview", "students", "parents", "staff", "classes", "years", "positions", "settings", "leave-review", "finance"];
...
const allowedPages = (context: Context): View[] => {
  ...
  const result: View[] = [];
  if (has("overview") && context.capabilities.includes("OPERATIONAL_QUEUE_READ")) result.push("overview");
  ...
  return <section className="school-context">
    ...
    {context && <><header className="school-context-heading"><p className="school-context-kicker">NGỮ CẢNH ĐANG LÀM VIỆC</p><h1 ref={heading} tabIndex={-1}>PassionEdu - {context.schoolName}</h1></header><nav className="school-context-navigation" aria-label="Điều hướng quản trị và nhân sự">
      ...
      {allowedPages(context).some((page) => ["years", "positions", "settings"].includes(page)) && <section className="school-context-nav-group"><button type="button" className="school-context-nav-group-toggle" aria-controls="settings-submenu" aria-expanded={expanded.settings} onClick={() => setExpanded((value) => ({ ...value, settings: !value.settings }))}>Cấu hình trường</button>{expanded.settings && <div id="settings-submenu">{([['years', 'Năm học'], ['positions', 'Chức danh & capability'], ['settings', 'Chính sách trường']] as const).filter(([page]) => allowedPages(context).includes(page)).map(([page, label]) => <button type="button" key={page} aria-current={view === page ? "page" : undefined} onClick={() => requestDestination({ schoolSlug: context.schoolSlug, page })}>{label}</button>)}</div>}</section>}
       {context.navigation.filter((item) => item.id === "overview" || item.id === "leave-review" || item.id === "finance").filter((item) => allowedPages(context).includes(item.id as View)).map((item) => <button type="button" key={item.id} className={`school-context-nav-item school-context-nav-${item.id}`} aria-current={view === item.id ? "page" : undefined} onClick={() => requestDestination({ schoolSlug: context.schoolSlug, page: item.id as View })}>{item.label}</button>)}</nav>
     <div className="school-context-workspace">{view === "overview" && allowedPages(context).includes(view) ? <OperationalQueueWorkspace key={context.schoolId} schoolId={context.schoolId} schoolName={context.schoolName} denied={handleWorkspaceDenied} /> : (["students", "parents", "staff", "classes", "years", "positions"] as View[]).includes(view) && allowedPages(context).includes(view) ? <RosterWorkspace schoolId={context.schoolId} schoolName={context.schoolName} denied={handleWorkspaceDenied} onStatusChange={updateRosterStatus} section={view as "students" | "parents" | "staff" | "classes" | "years" | "positions"} /> : view === "settings" && allowedPages(context).includes(view) ? <SettingsWorkspace schoolId={context.schoolId} schoolName={context.schoolName} denied={handleWorkspaceDenied} onStatusChange={updateSettingsStatus} /> : view === "leave-review" && allowedPages(context).includes(view) ? <LeaveReviewWorkspace schoolId={context.schoolId} schoolName={context.schoolName} denied={handleWorkspaceDenied} onStatusChange={setLeaveReviewStatus} /> : view === "finance" && allowedPages(context).includes(view) ? <FinanceWorkspace schoolId={context.schoolId} schoolName={context.schoolName} denied={handleWorkspaceDenied} onStatusChange={setFinanceStatus} /> : null}</div></>}
