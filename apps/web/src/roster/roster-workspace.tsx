...
type Pending = { id: string; schoolId: string; kind: 'school-year' | 'class' | 'rename' | 'student' | 'archive' | 'lifecycle' | 'parent' | 'parent-revoke' };
...
export function RosterWorkspace({ schoolId, schoolName, denied, onStatusChange }: { schoolId: string; schoolName: string; denied: () => void; onStatusChange?: (status: Status) => void }) {
  ...
  const loadYear = async (selected: string, requestGeneration = generation.current) => {
    setClasses([]); setStudents([]); setParentLinks({});
  ...
  useEffect(() => {
    ...
    currentSchool.current = schoolId; selectedYear.current = ''; stop(); setYears([]); setYearId(''); setClasses([]); setStudents([]); setParentLinks({}); setParent({});
