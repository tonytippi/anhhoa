...
export function OperationalQueueWorkspace({ schoolId, schoolName, denied }: { schoolId: string; schoolName: string; denied: () => void }) {
  ...
  const open = async (classId: string, status: "NOT_RECORDED" | "PENDING", attendanceOn = date) => {
    ...
    if (school.current !== schoolId || current !== request.current) return;
    if (deniedStatus(response.status)) return denied();
    if (!response.ok) throw new Error("Không thể tải danh sách vận hành.");
    setItems(((await response.json()) as { data: Items }).data);
  };
  useEffect(() => { school.current = schoolId; const params = new URLSearchParams(window.location.search); const nextDate = params.get("date") ?? today(); setDate(nextDate); void load(nextDate).then(async () => { const classId = params.get("classId"); const status = params.get("status"); if (classId && (status === "NOT_RECORDED" || status === "PENDING")) await open(classId, status, nextDate); }).catch((cause: Error) => setError(cause.message)); }, [schoolId]);
  return <section aria-labelledby="operational-queue-title"><h2 id="operational-queue-title">Tổng quan vận hành</h2><p>{schoolName} / Hàng đợi buổi sáng</p><label>Ngày <input type="date" value={date} onChange={(event) => { const next = event.target.value; setDate(next); window.history.replaceState(null, "", `${window.location.pathname}?date=${encodeURIComponent(next)}`); void load(next).catch((cause: Error) => setError(cause.message)); }} /></label>{error && <p role="alert">{error}</p>}{!queue && !error && <p aria-live="polite">Đang tải hàng đợi vận hành...</p>}{queue && <><p>{queue.attendanceOn} / {queue.explanation}</p>{!queue.operating ? <p>Ngày đã chọn không vận hành.</p> : <div className="table-scroll"><table><caption>Việc cần xử lý theo lớp</caption><thead><tr><th>Lớp</th><th>Thiếu điểm danh</th><th>Đơn nghỉ chờ duyệt</th></tr></thead><tbody>{queue.classes.length ? queue.classes.map((row) => <tr key={row.classId}><td>{row.className}</td><td><button type="button" onClick={() => void open(row.classId, "NOT_RECORDED").catch((cause: Error) => setError(cause.message))}>{row.attendanceGapCount} học sinh</button></td><td><button type="button" onClick={() => void open(row.classId, "PENDING").catch((cause: Error) => setError(cause.message))}>{row.pendingLeaveCount} đơn</button></td></tr>) : <tr><td colSpan={3}>Không có lớp trong phạm vi được cấp quyền.</td></tr>}</tbody></table></div>}{items && <div className="table-scroll"><table><caption>{items.className} / {items.status === "NOT_RECORDED" ? "Thiếu điểm danh" : "Đơn nghỉ chờ duyệt"}</caption><thead><tr><th>Học sinh</th><th>Mã học sinh</th></tr></thead><tbody>{items.students.length ? items.students.map((student) => <tr key={student.studentId}><td>{student.studentName}</td><td>{student.studentCode}</td></tr>) : <tr><td colSpan={2}>Không có học sinh phù hợp.</td></tr>}</tbody></table></div>}</>}</section>;
}
