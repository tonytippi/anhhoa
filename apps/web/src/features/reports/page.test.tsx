...
it('keeps the response month visible and retries a failed new month without repeating the toast', async () => {
  const fetch = vi.fn().mockImplementation((input: RequestInfo | URL) => { const url = input instanceof Request ? input.url : String(input); return Promise.resolve(new URL(url, window.location.origin).searchParams.get('billingMonth') === '2026-09' ? new Response(JSON.stringify({ error: { code: 'INTERNAL_SERVER_ERROR', message: 'Lỗi' } }), { status: 500 }) : new Response(JSON.stringify(report), { status: 200 })); });
  ...
  await waitFor(() => expect(fetch.mock.calls.filter(([input]) => { const url = input instanceof Request ? input.url : String(input); return new URL(url, window.location.origin).searchParams.get('billingMonth') === '2026-09'; })).toHaveLength(2)); expect(screen.getAllByText('Không thể tải tháng mới. Đang hiển thị dữ liệu trước đó.')).toHaveLength(1);
});
