...
const schoolClass = {
  id: "b2e36687-69b4-4e89-8ec0-141ff397837f",
  name: "Mầm 1",
  monthlyTuition: 1500000,
  status: "ARCHIVED",
...
const invoice = {
  id: "a2e36687-69b4-4e89-8ec0-141ff397837f",
  billingMonth: "2026-08",
  student: { name: "Bé An", nickname: "An" },
  schoolClass: { id: schoolClass.id, name: "Mầm 1" },
  status: "PENDING",
  total: 1500000,
...
function responseFor(
  ...
  return new Response(
    JSON.stringify(
      url.includes("/classes?") ? classesResponse : { data: invoices, meta },
    ),
    { status: 200 },
  );
}
function renderPage(entry = "/hoa-don?month=2026-08") {
  return render(
    <QueryClientProvider
      client={
...
it("renders snapshots, VND, status text, and URL-driven filters with archived class selection", async () => {
  const fetch = vi
    .fn()
    .mockImplementation((url: string) => Promise.resolve(responseFor(url)));
  vi.stubGlobal("fetch", fetch);
  renderPage();
  expect(
    await screen.findByRole("table", {
      name: "Danh sách hóa đơn tháng 08/2026",
    }),
  ).toBeVisible();
  expect(
    await screen.findByRole("option", { name: "Mầm 1 (Đã lưu trữ)" }),
  ).toBeVisible();
  ...
  await waitFor(() =>
    expect(
      fetch.mock.calls.some(([url]) =>
        String(url).includes(`classId=${schoolClass.id}`),
      ),
    ).toBe(true),
  );
});
...
it("normalizes a missing month URL before fetching invoices", async () => {
  const fetch = vi
    ...
    .mockImplementation((url: string) =>
      Promise.resolve(
        responseFor(url, [], { page: 1, pageSize: 20, total: 0, pageCount: 1 }),
      ),
    );
  vi.stubGlobal("fetch", fetch);
  renderPage("/hoa-don");
  await screen.findByLabelText("Tháng hóa đơn");
  await waitFor(() =>
    expect(
      fetch.mock.calls.filter(([url]) => String(url).includes("/invoices?")),
    ).toHaveLength(1),
  );
  expect(
    fetch.mock.calls.find(([url]) => String(url).includes("/invoices?"))![0],
  ).toContain("billingMonth=");
});
...
it("normalizes an invalid month URL before fetching invoices", async () => {
  ...
  vi.stubGlobal("fetch", fetch);
  renderPage("/hoa-don?month=2026-13&page=3");
  await screen.findByLabelText("Tháng hóa đơn");
  await waitFor(() =>
    expect(
      fetch.mock.calls.filter(([url]) => String(url).includes("/invoices?")),
  ...
  expect(invoiceUrl).toContain(`billingMonth=${currentMonth}`);
  expect(invoiceUrl).not.toContain("billingMonth=2026-13");
  expect(invoiceUrl).not.toContain("page=3");
});
...
it("resets page when changing every invoice filter", async () => {
  const fetch = vi
    ...
    .mockImplementation((url: string) => Promise.resolve(responseFor(url)));
  vi.stubGlobal("fetch", fetch);
  renderPage("/hoa-don?month=2026-08&page=3");
  await screen.findByRole("table", { name: "Danh sách hóa đơn tháng 08/2026" });
  const expectFirstPageRequest = async (
    action: () => void,
    parameter: string,
  ) => {
    const requestsBefore = fetch.mock.calls.filter(([url]) =>
    ...
    const invoiceUrl = String(
      fetch.mock.calls
        .filter(([url]) => String(url).includes("/invoices?"))
        .at(-1)![0],
    );
    expect(invoiceUrl).toContain(parameter);
    expect(invoiceUrl).not.toContain("page=3");
  };
  await expectFirstPageRequest(
    () =>
      fireEvent.change(screen.getByLabelText("Tháng hóa đơn"), {
  ...
  await expectFirstPageRequest(
    () =>
      fireEvent.change(screen.getByPlaceholderText("Tìm theo tên học sinh"), {
        target: { value: "Bé An" },
      }),
    "search=B%C3%A9+An",
  );
...
it("recovers from an out-of-range page instead of presenting it as an empty month", async () => {
  ...
  renderPage("/hoa-don?month=2026-08&page=3");
  expect(await screen.findByText("Trang này không còn hóa đơn.")).toBeVisible();
...
it("keeps the selected month and opens the invoice creation workflow", async () => {
  ...
  expect(screen.getByLabelText("Tháng hóa đơn")).toHaveValue("2026-08");
  fireEvent.click(screen.getByRole("button", { name: "Tạo hóa đơn tháng" }));
  expect(
    await screen.findByRole("heading", { name: "Tạo hóa đơn tháng" }),
  ).toBeVisible();
});
...
it("previews the current scope, creates it, and shows the batch result", async () => {
  const fetch = vi
    ...
    .mockImplementation((url: string, init?: RequestInit) => {
      ...
      if (String(url).includes("/invoices/batch-preview"))
        return Promise.resolve(
          new Response(
            JSON.stringify({
              data: {
      ...
      if (String(url).includes("/invoices/batch"))
        return Promise.resolve(
          new Response(
            JSON.stringify({
              data: {
  ...
  const createCall = fetch.mock.calls.find(([url]) =>
    String(url).endsWith("/invoices/batch"),
  )!;
...
it("previews selected classes when the class scope is chosen", async () => {
  const fetch = vi.fn().mockImplementation((url: string) => {
    ...
    if (String(url).includes("/invoices/batch-preview")) return Promise.resolve(new Response(JSON.stringify({ data: { eligibleCount: 1, skipped: { inactiveStudent: 0, missingClass: 0, archivedClass: 0, existingInvoice: 0 } } }), { status: 200 }));
    return Promise.resolve(responseFor(String(url)));
  });
  vi.stubGlobal("fetch", fetch);
  renderPage();
...
it("shows active-class loading errors for class scope and retries", async () => {
  let activeClassRequests = 0;
  const fetch = vi.fn().mockImplementation((url: string) => {
    if (String(url).includes("status=ACTIVE")) {
      activeClassRequests += 1;
      if (activeClassRequests === 1) return Promise.resolve(new Response(JSON.stringify({ error: { code: "UNAVAILABLE" } }), { status: 503 }));
      return Promise.resolve(new Response(JSON.stringify({ data: [schoolClass], meta: { page: 1, pageSize: 100, total: 1, pageCount: 1 } }), { status: 200 }));
    }
    return Promise.resolve(responseFor(String(url)));
  });
  vi.stubGlobal("fetch", fetch);
...
it("clears a zero-eligible preview and keeps creation disabled", async () => {
  ...
  fireEvent.change(screen.getByLabelText("Tháng tạo hóa đơn"), {
    target: { value: "2026-09" },
  });
  expect(
    screen.queryByText("Có 0 học sinh đủ điều kiện."),
...
it("reconciles a saved batch through 404 and pending without sending another POST", async () => {
  ...
  sessionStorage.setItem(
    "anhhoa.pending-invoice-batch",
    JSON.stringify({
      operationId,
      input: { billingMonth: "2026-08", allActiveClasses: true },
    }),
  );
  let lookups = 0;
  const fetch = vi
    ...
    .mockImplementation((url: string, init?: RequestInit) => {
      if (String(url).includes(`/operations/${operationId}`)) {
        ...
        if (lookups === 2)
          return Promise.resolve(
            new Response(
              JSON.stringify({ data: { operationId, state: "PENDING" } }),
              { status: 200 },
            ),
          );
        return Promise.resolve(
  ...
  expect(
    (fetch.mock.calls as unknown as [string, RequestInit | undefined][]).filter(
      ([url, init]) =>
        String(url).endsWith("/invoices/batch") && init?.method === "POST",
    ),
  ).toHaveLength(0);
