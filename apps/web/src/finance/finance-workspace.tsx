...
type Preview = {
  ...
  eligible: Array<{
    ...
    fullName: string;
    className: string;
    lines: Array<{ receivableId: string; grossAmount: string; discountAmount: string; netAmount: string; promotionEvaluation: { applications: Array<{ policyId: string; appliedDiscount: string }> } }>;
  }>;
  skips: Array<{
    studentId: string;
    studentCode?: string;
            ...
            <section aria-labelledby="preview-title">
              ...
              <table>
                ...
                <thead>
                  <tr>
                    <th>Học sinh</th>
                    <th>Lớp</th><th>Gross VND</th><th>Ưu đãi VND</th><th>Net VND</th><th>Lý do ưu đãi</th>
                  </tr>
                </thead>
                <tbody>
                  {preview.eligible.length ? (
                    preview.eligible.map((item) => (
                      <tr key={item.studentId}>
                        ...
                        <td>{item.className}</td><td style={{ textAlign: "right" }}>{vnd((item.lines ?? []).reduce((total, line) => total + BigInt(line.grossAmount), 0n).toString())}</td><td style={{ textAlign: "right" }}>{vnd((item.lines ?? []).reduce((total, line) => total + BigInt(line.discountAmount), 0n).toString())}</td><td style={{ textAlign: "right" }}>{vnd((item.lines ?? []).reduce((total, line) => total + BigInt(line.netAmount), 0n).toString())}</td><td>{(item.lines ?? []).flatMap((line) => line.promotionEvaluation.applications.map((application) => application.policyId)).join(", ") || "Không áp dụng"}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan={6}>Không có học sinh đủ điều kiện.</td>
                    </tr>
                  )}
                </tbody>
              </table>
