...
interface Parent { id: string; email: string; displayName: string | null; avatarUrl: string | null; }
...
function Shell({ parent, students, onLogout, current, children }: { parent: Parent; students: ParentStudent[]; onLogout: () => Promise<void>; current: 'home' | 'history'; children: React.ReactNode }): React.JSX.Element {
  ...
  useEffect(() => {
    const close = (event: MouseEvent | KeyboardEvent): void => {
      if (event instanceof KeyboardEvent) { if (event.key === 'Escape') setAccountOpen(false); return; }
      if (!account.current?.contains(event.target as Node)) setAccountOpen(false);
    };
    document.addEventListener('mousedown', close); document.addEventListener('keydown', close);
    return () => { document.removeEventListener('mousedown', close); document.removeEventListener('keydown', close); };
  }, []);
  const name = parent.displayName || parent.email;
  return <main><header className="app-header"><div className="brand"><span aria-hidden="true" className="brand-mark">✺</span><strong>Ánh Hoa</strong><span>Preschool</span></div><div className="account" ref={account}><button className="account-trigger" type="button" aria-expanded={accountOpen} aria-label={`Tài khoản của ${name}`} onClick={() => setAccountOpen((open) => !open)}><span className="account-name">{name}</span><Avatar parent={parent} /></button>{accountOpen && <section className="account-menu"><strong>{name}</strong><span>{parent.email}</span><div className="linked-students"><span>Các bé đang liên kết</span><ul>{students.map((student) => <li key={student.id}>{student.fullName}</li>)}</ul></div><button onClick={() => void onLogout()}>Đăng xuất</button></section>}</div></header>{offline && <p className="offline" role="status">Bạn đang ngoại tuyến. Dữ liệu có thể không mới.</p>}{children}<nav><Link to="/" aria-current={current === 'home' ? 'page' : undefined}>Trang chủ</Link><Link to="/history" aria-current={current === 'history' ? 'page' : undefined}>Lịch sử</Link></nav></main>;
}

function Avatar({ parent }: { parent: Parent }): React.JSX.Element {
  const [imageFailed, setImageFailed] = useState(false);
  const label = (parent.displayName || parent.email).trim();
  const initials = label.split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase();
  if (!parent.avatarUrl || imageFailed) return <span className="avatar avatar-fallback" aria-hidden="true">{initials}</span>;
  return <img className="avatar" src={parent.avatarUrl} alt="" onError={() => setImageFailed(true)} />;
}
...
function HomeContent({ groups, refreshing, onUnauthorized }: { groups: Array<{ student: ParentStudent; invoices: ParentInvoice[]; newestPosition: number }>; refreshing: boolean; onUnauthorized: () => void }): React.JSX.Element { if (!groups.length) return <section className="empty"><span aria-hidden="true" className="empty-mark">✓</span><h2>Không còn Hóa đơn cần thanh toán</h2><p>Nhà trường sẽ hiển thị các khoản cần xử lý tại đây.</p><Link to="/history">Xem lịch sử thanh toán</Link></section>; return <section aria-live="polite">{refreshing && <p className="refreshing">Đang cập nhật...</p>}{groups.map((group) => <section className="invoice-group" key={group.student.id}><h2>{group.student.nickname ?? group.student.fullName}</h2>{group.invoices.map((invoice) => <article className="invoice-card" key={invoice.id}><p className="month">{formatMonth(invoice.billingMonth)}</p><p className="student-name">Học sinh: {invoice.student.name}</p><p><span className="status">Cần thanh toán</span></p><strong>{formatMoney(invoice.total)}</strong>{invoice.paymentMethod === 'CASH' ? <p>Thanh toán tiền mặt tại nhà trường.</p> : <PaymentGuidance invoice={invoice} onUnauthorized={onUnauthorized} />}<Link className="button" to={`/invoices/${invoice.id}`}>Xem Hóa đơn</Link></article>)}</section>)}</section>; }
...
function InvoiceDetail({ parent, students, onUnauthorized, onLogout }: { parent: Parent; students: ParentStudent[]; onUnauthorized: () => void; onLogout: () => Promise<void> }): React.JSX.Element {
  ...
  return <Shell parent={parent} students={students} onLogout={onLogout} current="home"><h1>Chi tiết Hóa đơn</h1>{detail.isPending ? <HomeSkeleton /> : detail.error && !unauthorized ? <Retry client={client} label="Không thể tải Hóa đơn. Vui lòng thử lại." /> : detail.data && !revoked ? <InvoiceReadOnly invoice={detail.data} onUnauthorized={onUnauthorized} /> : null}</Shell>;
}

function InvoiceReadOnly({ invoice, onUnauthorized }: { invoice: ParentInvoice; onUnauthorized: () => void }): React.JSX.Element {
  const pending = invoice.status === 'PENDING';
  return <article className="invoice-detail"><header className="invoice-summary"><div><span className="invoice-label">Học sinh</span><strong>{invoice.student.name}</strong></div><span className={`status ${pending ? '' : 'completed'}`}>{pending ? 'Cần thanh toán' : 'Đã hoàn tất'}</span><p>{formatMonth(invoice.billingMonth)}</p></header><section className="invoice-items"><h2>Chi tiết các khoản thu</h2><ul>{invoice.items.length ? invoice.items.map((item) => <li key={`${item.position}-${item.description}`}><span>{item.description}</span><strong>{formatMoney(item.amount)}</strong></li>) : <li className="invoice-items-empty">Nhà trường chưa cung cấp chi tiết khoản thu.</li>}</ul></section><footer className="invoice-payment"><div><span>Phương thức</span><strong>{invoice.paymentMethod === 'CASH' ? 'Tiền mặt' : 'Chuyển khoản'}</strong></div><div className="invoice-total"><span>Tổng cộng</span><strong>{formatMoney(invoice.total)}</strong></div>{pending && invoice.paymentMethod === 'CASH' && <p>Thanh toán tiền mặt tại nhà trường.</p>}{pending && invoice.paymentMethod === 'TRANSFER' && <PaymentGuidance invoice={invoice} onUnauthorized={onUnauthorized} />}</footer></article>;
}
...
function Login(): React.JSX.Element { return <main className="login"><p>Ánh Hoa Preschool</p><h1>Đăng nhập</h1><p>Đăng nhập bằng tài khoản Google đã được nhà trường cấp quyền.</p><a className="button" href={`${(import.meta.env.VITE_API_URL ?? '/api').replace(/\/$/, '')}/parent/auth/google`}>Tiếp tục với Google</a></main>; }
