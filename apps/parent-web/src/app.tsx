...
function PaymentSheet({ invoiceId, payment, close, onUnauthorized, onDenied }: { invoiceId: string; payment: ParentPaymentResponse; close: () => void; onUnauthorized: () => void; onDenied: () => void }): React.JSX.Element {
  const dialog = useRef<HTMLDivElement>(null); const closeButton = useRef<HTMLButtonElement>(null); const [feedback, setFeedback] = useState(''); const [downloadError, setDownloadError] = useState(''); const [downloading, setDownloading] = useState(false);
