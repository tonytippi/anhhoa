...
describe('SettingsService validation and read scope', () => {
  ...
  it('reads profile and calendar as-of with the School tenant selector', async () => {
    ...
    const attendance = { id: 'attendance', effectiveFrom: new Date('2026-01-01T00:00:00Z'), photoEvidenceMode: 'REQUIRED', reason: 'An toàn', createdAt: new Date('2026-01-01T00:00:00Z') };
    const journal = { id: 'journal', effectiveFrom: new Date('2026-01-01T00:00:00Z'), reason: 'Bảo vệ dữ liệu', parentRetentionDaysAfterEnrollmentEnded: 30, acceptedImageMimeTypes: ['JPEG', 'PNG', 'WEBP'], maxImageSizeBytes: 10485760, imageCountLimit: null, createdAt: new Date('2026-01-01T00:00:00Z') };
    const prisma = { schoolProfileVersion: { findFirst: vi.fn().mockResolvedValue(profile) }, schoolCalendarVersion: { findFirst: vi.fn().mockResolvedValue(calendar) }, financePolicy: { findMany: vi.fn().mockResolvedValue([policy]) }, attendancePolicy: { findMany: vi.fn().mockResolvedValue([attendance]) }, handoverPolicy: { findMany: vi.fn().mockResolvedValue([attendance]) }, dailyJournalPolicy: { findMany: vi.fn().mockResolvedValue([journal]) }, leavePolicy: { findMany: vi.fn().mockResolvedValue([]) }, bankAccount: { findMany: vi.fn().mockResolvedValue([account]) } };
    const service = new SettingsService(prisma as never, authorization as never);
    ...
    await expect(service.read('identity', 'school-a', '2026-02-01')).resolves.toMatchObject({ attendancePolicy: { photoEvidenceMode: 'REQUIRED' }, handoverPolicy: { photoEvidenceMode: 'REQUIRED' }, dailyJournalPolicy: { parentRetentionDaysAfterEnrollmentEnded: 30, acceptedImageMimeTypes: ['JPEG', 'PNG', 'WEBP'], maxImageSizeBytes: 10485760, imageCountLimit: null } });
  ...
  it('requires evidence mode and reason before creating policy versions', async () => {
    const prisma = { attendancePolicy: { findFirst: vi.fn() }, handoverPolicy: { findFirst: vi.fn() }, dailyJournalPolicy: { findFirst: vi.fn() } };
    const service = new SettingsService(prisma as never, authorization as never);
    await expect(service.createAttendancePolicy('identity', 'school', crypto.randomUUID(), crypto.randomUUID(), { effectiveFrom: '2026-01-01', photoEvidenceMode: 'UNKNOWN', reason: 'A' })).rejects.toMatchObject({ status: 400, response: { fieldErrors: { photoEvidenceMode: expect.any(String) } } });
    await expect(service.createHandoverPolicy('identity', 'school', crypto.randomUUID(), crypto.randomUUID(), { effectiveFrom: '2026-01-01', photoEvidenceMode: 'OPTIONAL', reason: '' })).rejects.toMatchObject({ status: 400, response: { fieldErrors: { reason: expect.any(String) } } });
    await expect(service.createDailyJournalPolicy('identity', 'school', crypto.randomUUID(), crypto.randomUUID(), { effectiveFrom: '2026-01-01', reason: '' })).rejects.toMatchObject({ status: 400, response: { fieldErrors: { reason: expect.any(String) } } });
  });
  it('ignores client Daily Journal fact overrides and persists canonical values', async () => {
    const created = { id: 'journal', effectiveFrom: new Date('2026-01-01T00:00:00Z'), reason: 'Bảo vệ dữ liệu', parentRetentionDaysAfterEnrollmentEnded: 30, acceptedImageMimeTypes: ['JPEG', 'PNG', 'WEBP'], maxImageSizeBytes: 10485760, imageCountLimit: null, createdAt: new Date('2026-01-01T00:00:00Z') };
    const tx = { $queryRaw: vi.fn(), schoolMembership: { findFirst: vi.fn().mockResolvedValue({}) }, operation: { create: vi.fn().mockResolvedValue({ id: 'operation' }), update: vi.fn().mockResolvedValue({ id: 'operation', status: 'COMPLETED', outcome: created }) }, dailyJournalPolicy: { findFirst: vi.fn().mockResolvedValue(null), create: vi.fn().mockResolvedValue(created) }, auditRecord: { create: vi.fn() } };
    const prisma = { operation: { findFirst: vi.fn().mockResolvedValue(null) }, $transaction: vi.fn((work: (value: typeof tx) => unknown) => work(tx)) };
    const service = new SettingsService(prisma as never, authorization as never);
    await expect(service.createDailyJournalPolicy('identity', 'school', crypto.randomUUID(), crypto.randomUUID(), { effectiveFrom: '2026-01-01', reason: 'Bảo vệ dữ liệu', parentRetentionDaysAfterEnrollmentEnded: 1, acceptedImageMimeTypes: ['GIF'], maxImageSizeBytes: 1, imageCountLimit: 2 })).resolves.toMatchObject({ outcome: { parentRetentionDaysAfterEnrollmentEnded: 30, acceptedImageMimeTypes: ['JPEG', 'PNG', 'WEBP'], maxImageSizeBytes: 10485760, imageCountLimit: null } });
    expect(tx.dailyJournalPolicy.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ parentRetentionDaysAfterEnrollmentEnded: 30, acceptedImageMimeTypes: ['JPEG', 'PNG', 'WEBP'], maxImageSizeBytes: 10485760, imageCountLimit: null }) }));
  });
