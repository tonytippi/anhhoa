...
export class RosterController {
  constructor(private readonly auth: AuthService, private readonly roster: RosterService, private readonly parents?: ParentsService) {}
  ...
  @Get('students/:studentId/parents') async links(@Req() request: RequestLike, @Param('schoolId') schoolId: string, @Param('studentId') studentId: string) { return { data: await this.parents!.links(this.identity(request), schoolId, studentId) }; }
  @Post('students/:studentId/parents') async createParent(@Req() request: RequestLike, @Param('schoolId') schoolId: string, @Param('studentId') studentId: string, @Headers('idempotency-key') key: string, @Headers('x-operation-id') operationId: string, @Body() body: unknown) { return { data: await this.parents!.create(this.identity(request), schoolId, studentId, this.mutation(request, key), operationId ?? '', body) }; }
  @Post('student-parents/:linkId/revoke') async revokeParent(@Req() request: RequestLike, @Param('schoolId') schoolId: string, @Param('linkId') linkId: string, @Headers('idempotency-key') key: string, @Headers('x-operation-id') operationId: string) { return { data: await this.parents!.revoke(this.identity(request), schoolId, linkId, this.mutation(request, key), operationId ?? '') }; }
