...
import { audienceConfig, cookieSecure } from './auth.config.js';
...
describe('AuthController', () => {
  it('issues a non-Secure correlation cookie for a local HTTP callback', async () => {
    const app = await createApi(); await app.listen(0); const port = app.getHttpServer().address().port;
    vi.spyOn(app.get(AuthService), 'start').mockResolvedValue({ correlation: 'correlation-token', authorizationUrl: 'https://accounts.google.com/authorization' });
    const response = await fetch(`http://127.0.0.1:${port}/api/app/auth/google/start`, { redirect: 'manual' });
    await app.close(); const cookie = response.headers.get('set-cookie')!;
    expect(response.status).toBe(302); expect(cookie).toContain('app_oauth_correlation=correlation-token'); expect(cookie).not.toContain('Secure');
  });
  ...
  it.each(audiences.filter(([audience]) => audience !== 'parent'))('issues only the %s audience cookies at callback', async (audience) => {
    ...
    await app.close(); const cookie = response.headers.get('set-cookie')!;
    expect(response.status).toBe(302); expect(cookie).toContain(`${audience}_session=session-token`); expect(cookie).toContain(`${audience}_csrf=csrf-token`); expect(cookie).not.toContain('Domain='); expect(cookie).not.toContain('Secure');
  });
  ...
  it('clears only the selected host-only audience cookie on logout', async () => {
    ...
    await app.close(); const cookie = response.headers.get('set-cookie')!;
    expect(response.status).toBe(204); expect(cookie).toContain('app_session='); expect(cookie).toContain('app_csrf='); expect(cookie).toContain('HttpOnly'); expect(cookie).toContain('SameSite=Lax'); expect(cookie).not.toContain('Domain='); expect(cookie).not.toContain('ops_session='); expect(cookie).not.toContain('Secure');
  });
  ...
  it('issues Secure cookies when the callback URL uses HTTPS', async () => {
    const callbackUrl = process.env.APP_GOOGLE_CALLBACK_URL;
    try {
      process.env.APP_GOOGLE_CALLBACK_URL = 'https://api.passionedu.org/api/app/auth/google/callback';
      const app = await createApi(); await app.listen(0); const port = app.getHttpServer().address().port;
      const auth = app.get(AuthService);
      vi.spyOn(auth, 'start').mockResolvedValue({ correlation: 'correlation-token', authorizationUrl: 'https://accounts.google.com/authorization' });
      vi.spyOn(auth, 'callback').mockResolvedValue({ redirect: 'https://admin.passionedu.org', cookie: 'session-token', csrf: 'csrf-token' });
      const start = await fetch(`http://127.0.0.1:${port}/api/app/auth/google/start`, { redirect: 'manual' });
      const callback = await fetch(`http://127.0.0.1:${port}/api/app/auth/google/callback?state=state&code=code`, { redirect: 'manual' });
      const logout = await fetch(`http://127.0.0.1:${port}/api/app/auth/logout`, { method: 'POST', headers: { origin: 'http://localhost:5173', 'x-csrf-token': 'csrf-value', cookie: 'app_csrf=csrf-value' } });
      await app.close();
