...
const config = { csrfCookieName: 'csrf_token', oauthStateCookieName: 'oauth_state', oauthRedirectUrls: ['http://localhost:5173', 'http://localhost:5173/login'] } as never;
...
describe('AuthController', () => {
  it('issues an explicitly secure, httpOnly, Lax session cookie and preserves the state-validated redirect', async () => {
    ...
    expect(response.cookie).toHaveBeenCalledWith('session', 'signed-session', { secure: true, httpOnly: true, sameSite: 'lax', path: '/' });
