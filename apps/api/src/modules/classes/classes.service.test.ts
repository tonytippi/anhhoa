...
const record = { id: 'a2e36687-69b4-4e89-8ec0-141ff397837f', name: 'Mầm 1', monthlyTuition: 1500000n, status: ClassStatus.ACTIVE, createdAt: new Date('2026-01-01'), updatedAt: new Date('2026-01-01'), students: [] };
function prisma() {
  ...
  return { class: { findMany: vi.fn().mockResolvedValue([record]), count: vi.fn().mockResolvedValue(1), findUnique: vi.fn().mockResolvedValue(record), create: vi.fn().mockResolvedValue(record), update: vi.fn().mockResolvedValue(record) }, student: { count: vi.fn() }, $transaction: vi.fn(async (argument: unknown) => Array.isArray(argument) ? Promise.all(argument) : (argument as (client: typeof tx) => Promise<unknown>)(tx)), tx };
}

describe('ClassesService', () => {
  it('filters, trims search, and clamps pages beyond data', async () => {
    const db = prisma();
    const result = await new ClassesService(db as never).list({ page: 99, pageSize: 20, status: ClassStatus.ACTIVE, search: ' Mầm ' });
    expect(result.meta).toMatchObject({ page: 1, pageCount: 1, total: 1 });
    expect(db.tx.class.findMany).toHaveBeenCalledWith(expect.objectContaining({ skip: 0, where: { status: ClassStatus.ACTIVE, name: { contains: 'Mầm', mode: 'insensitive' } }, include: { students: { where: { status: StudentStatus.ACTIVE }, select: { id: true, fullName: true } } } }));
  });
  ...
  it('gets and updates a class, returning not found when absent', async () => {
    const db = prisma();
    const service = new ClassesService(db as never);
    await expect(service.get(record.id)).resolves.toMatchObject({ data: { id: record.id } });
    await expect(service.update(record.id, { name: 'Mầm 2', monthlyTuition: 100 })).resolves.toMatchObject({ data: { name: 'Mầm 1' } });
    expect(db.class.update).toHaveBeenCalledWith(expect.objectContaining({ data: { name: 'Mầm 2', monthlyTuition: 100n } }));
  ...
  it('uses a serializable transaction and blocks archive with active student count', async () => {
    ...
    expect(db.tx.class.findUnique).toHaveBeenCalledWith({ where: { id: record.id } });
  });

  it('soft archives and leaves an archived class idempotent', async () => {
    const db = prisma();
    const service = new ClassesService(db as never);
    await expect(service.archive(record.id)).resolves.toMatchObject({ data: { status: 'ARCHIVED', activeStudents: [] } });
