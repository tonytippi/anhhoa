    ...
    return this.mutate(grant.id, identityId, 'POST /api/ops/schools', key, operationId, { name, slug, ownerEmail, studentCodePrefix }, undefined, async (tx, operationId) => {
      ...
      const school = await tx.school.create({ data: { name, slug, studentCodePrefix, initialOwnerIdentityId: owner.id } });
       ...
       const ownerPosition = positions[0]!;
         await tx.positionCapabilityGrant.createMany({ data: ['SCHOOL_CONTEXT_READ', 'ACCESS_MANAGE', 'ROSTER_MANAGE', 'SETTINGS_MANAGE', 'FINANCE_MANAGE', 'CLASS_LEAVE_READ', 'LEAVE_REQUEST_DECIDE'].map((capability) => ({ schoolId: school.id, positionId: ownerPosition.id, capability })) });
         const financePosition = positions[2]!;
         await tx.positionCapabilityGrant.create({ data: { schoolId: school.id, positionId: financePosition.id, capability: 'FINANCE_MANAGE' } });
