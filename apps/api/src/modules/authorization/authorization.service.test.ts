...
describe('AuthorizationService audience projection', () => {
  it('does not expose access capability or navigation to teacher even for School Admin', async () => {
    ...
    await expect(service.resolve('identity', 'school', 'app')).resolves.toMatchObject({ capabilities: ['SCHOOL_CONTEXT_READ', 'ACCESS_MANAGE', 'ROSTER_MANAGE'], navigation: [{ id: 'overview' }, { id: 'access' }, { id: 'roster' }] });
  });
});
