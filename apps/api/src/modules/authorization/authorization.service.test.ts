...
describe('AuthorizationService audience projection', () => {
  it('does not expose access capability or navigation to teacher even for School Admin', async () => {
    ...
    await expect(service.resolve('identity', 'school', 'teacher')).resolves.toEqual({ schoolId: 'school', schoolName: 'Trường A', membershipId: 'membership', staffProfileId: 'staff', capabilities: ['SCHOOL_CONTEXT_READ'], navigation: [] });
