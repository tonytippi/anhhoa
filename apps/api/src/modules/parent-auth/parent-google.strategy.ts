...
export interface GoogleParentProfile { subject: string; email: string; displayName: string | null; }
...
export class ParentGoogleStrategy extends PassportStrategy(Strategy, 'parent-google') {
  ...
  validate(_accessToken: string, _refreshToken: string, profile: { id: string; emails?: Array<{ value: string; verified?: boolean }>; displayName?: string }): GoogleParentProfile {
    ...
    return { subject: profile.id.trim(), email: normalizedEmail, displayName: profile.displayName?.trim() || null };
  }
}
