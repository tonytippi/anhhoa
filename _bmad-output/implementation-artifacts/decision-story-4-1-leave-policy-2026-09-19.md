...
- `SCHOOL_ADMIN` và `FINANCE_MANAGER` có quyền duyệt hoặc từ chối đơn `PENDING`.
...
- Parent chỉ được tạo leave request cho Student có liên kết `StudentParent` đang `ACTIVE` trong School. Giáo viên được xem leave record của Student thuộc Class mà StaffProfile đã bind với membership/UserIdentity active và có assignment hiệu lực theo ngày liên quan.
...
- `StaffProfile` là nguồn nghiệp vụ. Staff có `employmentStatus` (`ACTIVE`/`INACTIVE`) và `staffType` (`TEACHER`/`STAFF`); School Admin chỉ phân công Class cho Staff đang `ACTIVE` và là `TEACHER`. Một Class có thể có nhiều StaffClassAssignment hiệu lực.
...
- Server authorise teacher leave read qua active SchoolMembership cua session, active StaffProfile binding, `employmentStatus: ACTIVE`, `staffType: TEACHER`, StaffClassAssignment hieu luc va Student placement hieu luc trong cung Class. Khong suy ra identity tu email hay tin Class/Student ID tu client.
