...
assert.equal(holiday.hidden, true);
assert.equal(holiday.dataset.dirty, undefined);
dialog().querySelector('[data-close]').click();
window.document.querySelector('[data-open-holiday-form]').click();
holiday.elements['holiday-name'].value = 'Nghỉ một ngày';
holiday.elements['holiday-start-date'].value = '2027-01-05';
holiday.elements['holiday-end-date'].value = '2027-01-05';
holiday.dispatchEvent(new window.Event('submit', { bubbles: true, cancelable: true }));
dialog().querySelector('[data-idempotent-submit]').click(); dialog().querySelector('[data-reconcile]').click(); dialog().querySelector('[data-return-operation-outcome]').click();
assert.match(window.document.querySelector('[data-holiday-rows] tr:last-child').textContent, /Nghỉ một ngày[\s\S]*1 ngày/);
dialog().querySelector('[data-close]').click();
window.document.querySelector('[data-open-holiday-form]').click();
...
assert.equal(holiday.querySelector('#holiday-end-date-error').hidden, false);
assert.equal(window.document.querySelectorAll('[data-holiday-rows] tr').length, 3);
