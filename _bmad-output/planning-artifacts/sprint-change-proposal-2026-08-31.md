...
- Nhiều trường độc lập dữ liệu; một User có membership và role khác nhau ở từng trường.
...
2. `User` là identity Google toàn platform. Quyền nội bộ là `SchoolMembership` + `SchoolRoleGrant`, không phải `Admin` global.
...
9. `SUPERADMIN_EMAIL` bootstrap một Platform Operator qua environment; provisioning nhận email `SCHOOL_ADMIN` đầu tiên, bind User khi họ login Google và không dùng password mặc định. School không xóa, chỉ suspend/reactivate.
...
16. Mỗi School có tối đa một `SchoolYear` active do `SCHOOL_ADMIN` quản lý. Calendar chung toàn trường. Class là cohort theo SchoolYear, nên wizard chuyển năm có thể map một phần trẻ từ lớp nguồn `3-4 tuổi` sang lớp đích mới `4-6 tuổi` và giữ/move các trẻ khác theo lựa chọn. Close-year đóng SchoolYear/kết thúc class assignment nhưng giữ enrollment lịch sử `ENROLLED`; wizard tạo enrollment mới `ENROLLED` khi gán lớp đích. `GRADUATED` chỉ dành cho trẻ thực sự hoàn thành/chuyển cấp rời trường.
...
21. Finance release đầu hỗ trợ CollectionRun `MONTHLY`, `ANNUAL` và `ONE_OFF`. School tự định nghĩa nhóm/khoản thu, đơn vị, giá, hoàn trả và rule áp dụng; mã khoản thu tùy chọn, unique khi có. PassionEdu không define sẵn behavior cho `Cơ sở vật chất`: School có thể tự tạo khoản này với unit `năm theo ngày nhập học`; Finance Manager điều chỉnh giá thực thu hoặc thêm adjustment trên invoice `DRAFT` có ghi chú/audit cho case nhập học giữa năm.
...
23. Tiền ăn tháng được trừ theo ngày nghỉ có phép khi đơn được Parent gửi trước 15:00 ngày ngay trước ngày nghỉ đầu tiên và được School duyệt; một đơn nhiều ngày hợp lệ áp dụng toàn bộ ngày đã duyệt. Nghỉ dài hạn/hủy dịch vụ được duyệt là nguồn refund, còn adjustment thủ công vẫn có ghi chú/audit. School hỗ trợ đăng ký học full thứ bảy theo effective dates; trẻ không đăng ký được tính học lẻ từ attendance thứ bảy, không charge trùng ngày đã được gói bao phủ. Xe đưa đón deferred.
24. Khi chưa có nguồn dữ liệu finance, Finance Manager/SCHOOL_ADMIN nhập quantity thủ công trong invoice `DRAFT`; khi nguồn đã có, họ vẫn được override quantity/giá hoặc adjustment trong `DRAFT` với ghi chú/audit và source snapshot còn giữ. Staff profile không cấp login: chỉ User có SchoolMembership/role active truy cập portal. Import dữ liệu lịch sử vận hành deferred thành onboarding workstream riêng sau core.
25. Parent chỉ mutation leave request cho trẻ được ủy quyền: trước 15:00 ngày trước ngày nghỉ đầu tiên là `AUTO_APPROVED`; từ 15:00 cần School Admin/Finance Manager duyệt. School cấu hình `AttendancePolicy.photoEvidenceMode`: `REQUIRED` buộc giáo viên chụp/upload ảnh trước `PRESENT`, `OPTIONAL` cho phép không ảnh; Parent chỉ nhận notification event, không xem evidence ảnh release đầu. Không xác nhận attendance ngày nghỉ/lễ. `PRESENT` trên ngày có leave request đánh dấu conflict và thắng deduction ngày đó. Thứ bảy lẻ chỉ charge `PRESENT` đã xác nhận; gói thứ bảy chỉ trừ bởi leave request đủ điều kiện không có `PRESENT`. Giáo viên xác nhận giờ trả trẻ thực tế. Late pickup dùng `pickedUpAt` và School LatePickupPolicy cutoff/grace/block; ví dụ cutoff 17:30 không grace, block 30 phút thì `17:31` là một block. API tính/snapshot, Finance chỉ override `DRAFT` có ghi chú.
...
| --- | --- | --- | --- |
| Epic 1: truy cập Admin | done | `ADMIN_EMAILS` và `Admin` global không còn hợp lệ | Supersede; giữ Google OAuth/cookie/CSRF pattern nhưng đổi sang User + membership + chooser |
...
| Epic 5: Parent links | in-progress nhưng stories done | Quan hệ nhiều-nhiều giữ lại, nhưng identity/link phải school scoped và role-aware | Rebuild cùng User/School/Parent design |
...
| --- | --- | --- | --- |
| Admin PRD §§1, 2.2, 4.1 | Một dashboard duy nhất, Admin global qua allowlist, không có Staff/Parent | Platform multi-school; User global, membership/role per-school; Admin/Staff và Parent là surface riêng | Cần tenant isolation và quyền khác nhau theo trường |
...
| AD-9 | Operation scope Admin + route + key | Operation scope school + actor membership + route + key; audit records User and membership |
| Parent AD-13 to AD-18 | Parent identity separate from Admin/Staff; parent invoice lifecycle old | User identity can be cross-persona; Parent authorization is `StudentParent` scoped to school; portal reads new finance read model |
...
1. **Platform multi-school, identity and authorization**
   - PassionEdu platform, School tenant, User, membership, role grants, chooser, switcher, Admin/Staff portal and narrow Platform Operations provisioning.
...
4. **Attendance, leave, service registration, handover and late pickup**
   - Attendance/leave approval, Saturday service registration and teacher-recorded handover are required sources for meal, Saturday and late-pickup finance rules. All quantity calculations remain API-owned.
...
| --- | --- | --- |
| E1. PassionEdu identity, access and control plane | User login, `SUPERADMIN_EMAIL` bootstrap, School, pending owner membership by email, role grant, school chooser/switcher, tenant guard, Platform Operations provisioning, cross-tenant tests | none |
...
| E4. Attendance, leave, service registration and handover | School-configurable attendance photo evidence with two-month Staff/Admin-scoped access and Parent event notification, Parent leave request auto-approval before 15:00/manual approval after, calendar gate/conflict handling, Saturday service enrollment, late-pickup source projections and anti-double-charge rules | E1, E2 |
| E5. Collection runs and invoice obligations | `MONTHLY`/`ANNUAL`/`ONE_OFF` charge rules, ready/preview/generate `DRAFT`, meal deduction/Saturday/late-pickup quantity sources, add an eligible student after batch generation, per-invoice review/override/bank selection/issue, snapshots, issue/void rules | E2, E3, E4 |
...
Do not begin E3-E7 before E1 has proven route-level tenant isolation. Do not begin E4 before SchoolYear/enrollment semantics are in E2. Do not begin E5 before E4 has defined the finance source snapshots.

### 5.3 Mandatory architecture acceptance criteria

- Same User can hold different roles at schools A/B; every API call authorizes the selected school membership/capability.
