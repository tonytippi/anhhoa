...
- **Correction (2026-09-21, Finance Admin MVP):** Before the attendance/handover release, `finance` may persist a manual Finance-source snapshot only on a same-School Student's DRAFT Invoice line. It contains an optional service date, attendance status, picked-up time and/or late-care minutes, plus mandatory actor, reason, timestamp and provenance. Finance validates StudentEnrollment, DRAFT state, business date, whole-VND quantity/unit price and the tenant graph. This snapshot is explanatory input only: it does not calculate quantity, price, fee, discount or Invoice total, and becomes immutable at issue. The attendance, handover, Teacher and Parent modules are not dependencies of this command; later operational records never overwrite or re-price an issued Finance-source snapshot.

- **Correction (2026-09-24, CollectionRun template):** Finance Admin MVP does not use `ChargeRule`, `FIXED`/`MANUAL` rule modes, School/Class/Student precedence or automatic charge selection. `FINANCE_MANAGE` is a dedicated SchoolPosition capability for the Finance catalog, monthly CollectionRun template, DRAFT line and issue commands. A `DRAFT` CollectionRun owns `CollectionRunTemplateLine` records: each references one active same-School Receivable at most once, has a strictly positive integer quantity and uses the catalog default unit price. Meal Receivables use unit `ngày` and default price `35.000 VND`; quantity `22` derives `770.000 VND`. The server derives every template amount/total in `BIGINT`, audits idempotent template mutation and fingerprints canonical template/catalog facts with preview. READY/GENERATED/CLOSED rejects template mutation. Generate locks/revalidates/snapshots template facts and derived lines into every eligible Invoice DRAFT atomically; it never accepts client lines/amounts/totals or creates empty DRAFT Invoice. A Student added after GENERATED uses only the immutable run template snapshot, never template/catalog live data. Template and Invoice DTOs display amount descending with a stable server tie-breaker; no position, reorder or user preference is persisted. Per-Invoice DRAFT edits, including audited override/quantity exceptions, never rewrite template or another Invoice. ChargeRule automation is a later Finance enhancement that must not rewrite generated or issued lines without a separate approved contract.

- **Correction (2026-09-24, PromotionPolicy Pha 1b):** After the CollectionRun template gate, `finance` owns same-School `PromotionPolicy`, effective-dated version, Receivable target, Student assignment and Invoice application snapshot for normal per-Receivable discount. API evaluates effective facts in preview/generate and re-evaluates inside Issue transaction; all are fingerprint inputs and a changed result requires review rather than silent issue. Fixed VND precedes percentage; priority, exclusivity and a stable server tie-breaker are deterministic; discount is capped at that target line's gross and cannot create a negative line, generic credit or balance. Every issued application snapshots gross, discount, net, policy/version/target/outcome and assignment provenance; later live facts never rewrite it. `PREPAID_COVERAGE`, future-period facts, coverage issuance, Receipt, settlement, carry and refund remain Epic 6-only.
