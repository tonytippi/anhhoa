...
- **Correction (2026-09-24, CollectionRun template):** Finance Admin MVP does not use `ChargeRule`, `FIXED`/`MANUAL` rule modes, School/Class/Student precedence or automatic charge selection. `FINANCE_MANAGE` is a dedicated SchoolPosition capability for the Finance catalog, monthly CollectionRun template, DRAFT line and issue commands. A `DRAFT` CollectionRun owns ordered `CollectionRunTemplateLine` records: each references one active same-School Receivable at most once, has a strictly positive integer quantity and uses the catalog default unit price. The server derives every template amount/total in `BIGINT`, audits idempotent template mutation and fingerprints canonical template/catalog facts with preview. READY/GENERATED/CLOSED rejects template mutation. Generate locks/revalidates/snapshots template facts and derived lines into every eligible Invoice DRAFT atomically; it never accepts client lines/amounts/totals or creates empty DRAFT Invoice. Per-Invoice DRAFT edits, including audited override/quantity exceptions, never rewrite template or another Invoice. ChargeRule automation is a later Finance enhancement that must not rewrite generated or issued lines without a separate approved contract.
