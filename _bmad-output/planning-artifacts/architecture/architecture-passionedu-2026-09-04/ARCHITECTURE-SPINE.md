...
- **Correction (2026-09-21, manual Invoice MVP):** Finance Admin MVP does not use `ChargeRule`, `FIXED`/`MANUAL` rule modes, School/Class/Student precedence or automatic charge selection. `FINANCE_MANAGE` is a dedicated SchoolPosition capability for the Finance catalog, monthly CollectionRun, DRAFT line and issue commands. A CollectionRun selects eligible Students and creates an empty idempotent DRAFT Invoice; Finance adds, edits or removes its DRAFT lines by selecting an active same-School Receivable and entering a strictly positive integer quantity. The server takes the catalog default unit price or an audited authorized override, derives every line and Invoice total in `BIGINT`, and rejects zero/negative/fractional quantity, non-positive price and client-supplied totals. The absence of a line means the receivable does not apply. ChargeRule automation is a later Finance enhancement that must not rewrite existing DRAFT or issued lines without a separate approved contract.
