...
Admin and Ops use a desktop-first shell with a persistent sidebar at `>= 1024px`, content gutter {spacing.gutter-desktop}, and responsive stacked cards below that width. Daily operational screens are table-first: keep the route heading compact, place primary action beside it, use a short summary strip only when it changes the next action, then show filters and a dense data table. In Admin, the table is always the primary management surface. A button that starts a bounded business action, short create/edit form or confirmation opens a one-level modal rather than expanding a form or secondary workspace inline. Filters, sorting, pagination, tabs and direct utility controls remain in place; URL-addressable detail, audit/history, invoice review and multi-step import, mapping, preview or reconciliation workflows remain dedicated routes. Teacher and Parent are mobile-first: Teacher prioritizes class/day action controls; Parent uses {spacing.gutter-mobile}, single-column sections and a maximum reading width of 640px. Tables retain horizontal scroll with sticky identifying columns on small viewports; never compress money or status into unreadable columns.
...
| Put the primary action beside a compact route heading and row actions in the final table column | Repeat route actions or explanatory banners after an operator has entered a detail view |
| Use prose only for exceptions, validation, confirmation and audit disclosures | Add instructional paragraphs to every row, metric or familiar daily task |
