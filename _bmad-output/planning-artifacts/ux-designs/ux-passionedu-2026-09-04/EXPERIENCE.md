...
| Invoice review and receipt | Finance | Finance Admin MVP review is a contextual destination after selecting a Student and Invoice from CollectionRun detail. On a `DRAFT`, Finance may add a `MANUAL` line with service date, attendance status, picked-up time and/or late-care minutes as an audited explanatory reference; it must enter reason and server-validated VND quantity/price, and the UI never suggests an automatic fee. Receipt, carry and revision appear only in the later Finance settlement release. |
