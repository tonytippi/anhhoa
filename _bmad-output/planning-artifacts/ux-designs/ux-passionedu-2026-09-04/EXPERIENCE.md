...
| Khoản thu / Đợt thu | Finance | Finance Admin MVP có hai destination: `Khoản thu` là catalog active/inactive; `Đợt thu` là CollectionRun list/detail để cấu hình template khoản thu chung, chọn Student, preview và generate Invoice DRAFT có sẵn dòng template. Review/issue Invoice là deep destination theo Student từ Đợt thu, không phải sidebar item. Không có Teacher, Parent, service, promotion, Receipt, carry, settlement KPI hay Payroll dependency. |
...
| CollectionRun list and detail | Finance | Finance sidebar opens `Khoản thu` and `Đợt thu`, never Invoice review or Receipt. Default `Đợt thu` view is a compact table of operating collection runs with filter and create action. Trong DRAFT, Finance thêm/bỏ/sắp xếp Receivable active một lần vào template chung và đặt quantity nguyên dương; giá/amount là server-returned. Selecting a run opens template và Student table with server eligibility/status and contextual Invoice-review action. Preview/generate snapshots the common template; per-Student exception chỉ chỉnh trong Invoice DRAFT có audit. Detail hides landing actions and retains only run title plus Back. Timeout goes to Operation reconciliation. |
| Invoice review and receipt | Finance | Finance Admin MVP review is a contextual destination after selecting a Student and Invoice from CollectionRun detail. On a `DRAFT`, Finance adds an active catalog receivable, enters a positive quantity, may use an authorized audited price override, and removes a line when it does not apply. The server returns each amount and total; the UI has no zero-value line or automatic fee suggestion. An optional attendance/late-care reference is explanatory and requires reason. Receipt, carry and revision appear only in the later Finance settlement release. |
| Promotion policy and debt transfer | School Admin, Finance | Finance settlement enhancement after the Finance Admin MVP manages effective-dated policy versions with targets, quantity/unit, discount, fulfillment and stacking. After offline Parent agreement, a Finance Manager or School Admin selects an eligible `PREPAID_COVERAGE` policy version for one or more Students in a monthly-run preview; its start month is the run billing month. Debt transfer names source and target obligation; it is an auditable movement, not an editable balance. |
...
| --- | --- | --- | --- |
| CollectionRun `DRAFT` | Editable SchoolYear, billing month and Student selection | Edit, preview | Generate is unavailable until server accepts `READY`. |
...
| Invoice `DRAFT` | Editable catalog lines, positive quantity, authorized price override and explanatory reason | Add/edit/remove line, choose active BankAccount, issue | Server blocks inactive/wrong-School receivable, zero/invalid price or quantity and lifecycle conflict. |
| Invoice `ISSUED` | Immutable obligation/Payment instruction snapshot and outstanding | Read, record one actual Receipt or prepare correction | Content edit and BankAccount change are unavailable. |
...
1. Minh enters a School-scoped CollectionRun wizard, chooses the period and selects eligible Students.
...
5. Minh reviews a draft, adds/removes catalog lines, records a reason for an override or explanatory reference, selects an active BankAccount and issues it.
