...
| Khoản thu / Đợt thu | Finance | Finance Admin MVP: catalog, CollectionRun setup, preview, generate, DRAFT review with audited manual attendance/late-care reference, and issue review. No Teacher, Parent, service, promotion, or Payroll dependency. |
...
| CollectionRun list and detail | Finance | Finance sidebar opens only list/configuration workspaces, never a fixture Invoice or Receipt. Default view is a compact table of operating collection runs with filter and create action. Selecting a run opens a Student table with eligibility/status and an Invoice action. Finance selects Students; preview/generate remains server-authoritative and creates no charge line. Detail hides landing actions and retains only run title plus Back. Timeout goes to Operation reconciliation. |
