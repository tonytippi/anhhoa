...
| Khoản thu / Ưu đãi / Đợt thu | Finance | `Khoản thu` là catalog active/inactive; Pha 1b thêm `Ưu đãi` để quản lý policy/version, target Receivable và Student assignment; `Đợt thu` preview/generate Invoice DRAFT có server-derived per-line promotion application. Review/issue Invoice là deep destination theo Student, không phải sidebar item. Không có Teacher, Parent, service, Receipt, carry, settlement KPI hay Payroll dependency trong Pha 1b. |
...
| CollectionRun list and detail | Finance | Finance sidebar opens `Khoản thu`, `Ưu đãi` and `Đợt thu`, never Invoice review or Receipt. Preview returns gross, applied/not-applied reason, discount and net per Student/Receivable; changing template/catalog/policy/version/target/assignment requires a new server preview. Generate snapshots common template plus applications; Student thêm sau GENERATED dùng snapshot run, không đọc template/catalog live. |
| Invoice review and issue | Finance | DRAFT review is contextual after CollectionRun. Finance may add/remove active catalog line or make an audited allowed base-price exception, then requests server refresh; it cannot edit policy-derived discount. The screen discloses server gross, policy/version, discount and net per line. Issue rechecks and snapshots applications; Receipt/carry/revision remain later settlement behavior. |
| Promotion policy and debt transfer | School Admin, Finance | Pha 1b quản lý effective-dated policy versions, Receivable targets, fixed-VND/percentage, priority/exclusivity và generic Student assignment with reason/audit. `PREPAID_COVERAGE` selection after offline Parent agreement and debt transfer remain later Finance settlement enhancement. |
