...
| Cấu hình khoản thu | `admin/receivable-configuration.html` | Ba tab có thể mở trực tiếp bằng liên kết: Danh sách khoản thu, Bảng khoản thu theo lớp và Chính sách ưu đãi. Danh sách khoản thu cho xem ngay phạm vi áp dụng của mỗi khoản. Từ Thêm mới hoặc Sửa trong danh sách, bản mẫu mở biểu mẫu của khoản thu đó; biểu mẫu gồm thông tin, phạm vi áp dụng theo Toàn trường/Lớp/Học sinh và cách tính số lượng. Bảng khoản thu theo lớp là ma trận có lọc theo kỳ/lớp: mỗi hàng là học sinh, mỗi cột là mọi khoản thu đang áp dụng cho lớp. Khoản thu theo phạm vi chỉ đọc trong bảng; Kế toán bật hoặc bỏ dịch vụ chọn riêng, ví dụ Học thứ 7, cho từng học sinh. Chính sách ưu đãi là danh mục versioned: một hoặc nhiều khoản thu target, số lượng/đơn vị, mức giảm, cách áp dụng, kết hợp và hiệu lực; policy gán học sinh là cơ chế tổng quát, không suy luận quan hệ gia đình. Bảng không đặt giá riêng hoặc thay hóa đơn đã phát hành. |
...
- Giờ trả trẻ là dữ liệu vận hành tham chiếu có audit; không có cấu hình, thống kê block/VND hay tự động tính phí đón muộn. Finance chỉ có thể thêm dòng `MANUAL` đã audit vào Invoice `DRAFT` khi cần.
