...
| Đợt thu | `admin/invoice-generation.html` | Landing danh sách lịch sử đợt thu với filter fixture, statistics server-confirmed, kỳ/loại/trạng thái, Invoice tạo/bỏ qua, gross/ưu đãi/net và thời điểm tính. Form tạo run mở theo progressive disclosure, chỉ đề xuất Receivable active/đúng scope/còn hiệu lực rồi nhận preview authoritative trước khi generate idempotent. `PREPAID` là luồng dedicated chỉ School Admin, không phải khoản thêm của run thường và chỉ có coverage sau exact PAID settlement. |
