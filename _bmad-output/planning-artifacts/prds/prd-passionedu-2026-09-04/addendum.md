...
- Finance Admin MVP dung Receivable catalog va dong Invoice DRAFT do Finance chon, khong dung ChargeRule, `FIXED`/`MANUAL`, precedence hay auto-charge. Dong chon Receivable active same-School, quantity nguyen duong va default/override unit price VND co audit; server tinh amount/total va reject quantity `0`, gia khong duong, float hay client total. ChargeRule automation la enhancement sau MVP.
...
- Finance Admin MVP: Receivable catalog lifecycle, monthly CollectionRun Student selection/preview, empty DRAFT Invoice generate idempotent, positive DRAFT line add/edit/remove, server-owned VND totals, price-override/source audit, issued snapshot immutability, tenant isolation and no Teacher/Parent/attendance/service/promotion/Payroll dependency.
