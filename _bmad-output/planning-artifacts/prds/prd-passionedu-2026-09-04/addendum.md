...
- Finance Admin MVP dung Receivable catalog va `CollectionRunTemplateLine` tren CollectionRun `DRAFT`, khong dung ChargeRule, `FIXED`/`MANUAL`, precedence hay auto-charge. Template chon moi Receivable active same-School toi da mot lan, co position va quantity nguyen duong do Finance nhap, dung default unit price catalog; server tinh amount/total, audit mutation va reject quantity `0`, gia khong duong, float hay client total. Generate snapshot template facts/amount vao Invoice DRAFT cua moi Student eligible; sau do Finance co the dieu chinh line rieng tung Invoice DRAFT theo audit. ChargeRule automation la enhancement sau MVP.
