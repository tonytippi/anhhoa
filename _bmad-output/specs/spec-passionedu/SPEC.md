...
- **CAP-4**
  - **intent:** Finance users can configure receivables and versioned promotion policies, review authoritative monthly CollectionRun previews and create draft obligations for eligible Students. A policy can target one or more Receivables with typed quantity/unit rules, discounts, fulfillment and stacking; after a direct Parent agreement, a Finance Manager or School Admin can select an eligible `PREPAID_COVERAGE` policy for one or more Students while reviewing that monthly run.
  - **success:** A Student has at most one Invoice in a CollectionRun; generation reconciles through its Operation and never duplicates an Invoice after retry or timeout. Server evaluation is snapshotted and re-evaluated before issue. The selected Invoice includes only the policy-covered future periods and non-covered Receivables of the open billing month. Coverage issues only after that Invoice closes `EXACT`, has immutable per-period service intervals and paid source/policy-version provenance, skips only its covered Student receivable-period facts, while other eligible charges remain billable.
- **CAP-5**
