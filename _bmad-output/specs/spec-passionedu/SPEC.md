...
- **CAP-4**
  - **intent:** Finance Admin MVP users manage School-scoped receivables, configure common positive-quantity template lines for monthly CollectionRuns, select eligible Students, and generate populated DRAFT obligations before auditing per-Student exceptions and issuing a snapshot obligation, without Teacher or Parent portals.
  - **success:** A Student has at most one Invoice in a CollectionRun; generation reconciles through its Operation and never duplicates an Invoice or template line after retry or timeout. The API alone derives template/Invoice line totals from active catalog selections, positive integer quantities and authorized unit-price overrides. Template change invalidates preview, generated/issued snapshots remain immutable, and zero lines/client total injection are rejected.
- **CAP-5**
...
- Finance Admin MVP uses an active/inactive School-scoped Receivable catalog and a dedicated `FINANCE_MANAGE` capability. A DRAFT CollectionRun template line references one active same-School Receivable, is unique in that run, carries a strictly positive integer quantity and catalog default unit price; API derives VND amount/total as PostgreSQL `BIGINT`, fingerprints template/catalog facts in preview and snapshots them into generated Invoice DRAFT lines. Per-Invoice DRAFT adjustment may use an audited authorized override but never rewrites the run template or another Invoice. ChargeRule, automated School/Class/Student applicability, policy evaluation and automatic charge creation are deferred Finance automation.
