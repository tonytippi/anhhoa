...
sources:
  ...
  - ../../planning-artifacts/sprint-change-proposal-2026-09-24-collection-run-template.md
---
...
- **CAP-4**
  - **intent:** Finance users manage School-scoped receivables, common monthly CollectionRun templates and per-Receivable promotion policies/Student assignments, then generate DRAFT obligations with server-derived discount applications before issuing an immutable snapshot, without Teacher or Parent portals.
  - **success:** A Student has at most one Invoice in a CollectionRun; generation reconciles through its Operation and never duplicates an Invoice, template line or promotion application after retry or timeout. API alone derives gross, discount and net from active catalog/template facts, positive integer quantities and effective same-School policy/version/target/assignment facts. Fixed VND precedes percentage, discount is capped at target gross, changed policy facts stale preview, Issue re-evaluates in its transaction, and generated/issued snapshots remain immutable.
- **CAP-5**
...
- Finance uses an active/inactive School-scoped Receivable catalog and `FINANCE_MANAGE`. A DRAFT CollectionRun template line references one active same-School Receivable, is unique in that run, carries positive integer quantity/default price; API derives VND `BIGINT` gross/total and snapshots template facts. Pha 1b adds versioned School-scoped `PromotionPolicy`, same-School Receivable target and effective `StudentPromotionAssignment`; API evaluates per line at preview/generate and again inside Issue, snapshots application/version/target/discount/net/provenance, and fingerprints every relevant policy fact. Fixed VND precedes percentage, priority/exclusivity is deterministic, and discount never exceeds target gross. No browser sets discount/total/eligibility. ChargeRule and automatic School/Class/Student applicability remain deferred.
- `PREPAID_COVERAGE` is a later Epic 6 PromotionPolicy fulfillment mode within a normal `MONTHLY` CollectionRun; coverage issues only after its Invoice closes `EXACT`. It is not a Pha 1b route, persistence model or release gate. High-impact cookie mutations require origin validation, double-submit CSRF, idempotency and Operation reconciliation.
