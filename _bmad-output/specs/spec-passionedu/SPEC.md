...
- **CAP-4**
  - **intent:** Finance users can configure receivables, calendar-month CollectionRuns and active prepaid-payment promotion programs with fixed consecutive-month terms, review authoritative previews and create draft obligations for eligible Students; after a direct agreement with a Parent, a School Admin can select a program and start month on a dedicated `PREPAID` CollectionRun.
  - **success:** A Student has at most one Invoice in a CollectionRun; generation reconciles through its Operation and never duplicates an Invoice after retry or timeout. An issued prepaid-payment coverage has immutable per-period service intervals and paid source provenance, skips only its covered Student receivable-period facts, while other eligible charges remain billable.
- **CAP-5**
